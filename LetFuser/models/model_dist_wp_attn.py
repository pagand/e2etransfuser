...
class letfuser(nn.Module): #
    def __init__(self, config, device):
        ...
        self.attn_wp = nn.MultiheadAttention(656, 4, batch_first = True)
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in, gt_command ):#, gt_ss, gt_redl:
       ...
       # fuse = hx.clone()#NEW
        ...
        for _ in range(self.config.pred_len):
            ...
            concat_feat_wp = torch.cat([hx,tls_bias], dim=1)
            concat_feat_wp_attn, concat_attn = self.attn_wp(concat_feat_wp,concat_feat_wp,concat_feat_wp)

            d_xy = self.pred_dwp(concat_feat_wp+concat_feat_wp_attn) #control v5
            xy = xy + d_xy
            out_wp.append(xy)

            # distilation single task
            D_hx = self.D_gru(ins, D_hx)
            D_concat_feat_wp = torch.cat([D_hx,D_tls_bias], dim=1)
            D_concat_feat_wp_attn, D_concat_attn = self.attn_wp(D_concat_feat_wp,D_concat_feat_wp,D_concat_feat_wp)

            D_d_xy = self.D_pred_dwp(D_concat_feat_wp+D_concat_feat_wp_attn) #control v5
            D_xy = D_xy + D_d_xy
