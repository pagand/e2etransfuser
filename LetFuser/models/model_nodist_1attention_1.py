...
#        attn_mixed, attn_tensor = self.attn_mixer(attn, attn_pooled, attn_pooled)
        ...
        attn, attn_tensor = self.attn(x_cat,x_cat,12,41)
        x = torch.cat((res,res_p),dim=1) + self.drop_path(attn)# + self.drop_path(F.sigmoid(attn_mixed))
...
class letfuser(nn.Module): #
    def __init__(self, config, device):
        ...
        if config.attn:
            ...
            self.adjust_size = nn.Linear(492,480)
        else:
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in, gt_command ):#, gt_ss, gt_redl:
       ...
       # fuse = hx.clone()#NEW
        ...
        x = self.adjust_size(x.permute(0,2,1))

        x = rearrange(x , 'b c (h w) -> b c h w', h=H,w=W)
        hx = self.attn_neck(x)
#        hxp = self.attn_neck_pooled(x.reshape(bs,-1,Hp,Wp)) 
        hx = torch.cat([hx, measurement_feature], dim=1) 
        fuse = hx.clone()#NEW
