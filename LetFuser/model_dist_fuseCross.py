...
class Fusion_Block(nn.Module):
    ...
    def forward(self, features_RGB,features_SC, h, w):
        ...
        x = self.norm1(features_RGB)
        y = self.norm2(features_SC)
...
class letfuser(nn.Module): #
    def __init__(self, config, device):
        ...
        self.fuse_BN = nn.BatchNorm2d(config.n_fmap_b1[3][-1])
        ...
        self.RGB_resize = nn.Linear(384,112)
        #PID Controller
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in, gt_command ):#, gt_ss, gt_redl:
        ...
        measurement_feature = self.measurements(torch.cat([next_route, velo_in.unsqueeze(-1), F.one_hot((gt_command-1).to(torch.int64).long(), num_classes=6)], dim=1))
     #   fuse = self.fuse_BN(torch.cat([RGB_features8, SC_features5], dim=1))
     #   print(self.RGB_resize(RGB_features8.permute(0,2,3,1)).shape)
     #   print(SC_features5.shape)
     #   fuse = self.fuse_BN(self.RGB_resize(RGB_features8.permute(0,2,3,1)).permute(0,3,1,2)+ SC_features5)

        RGB_features8_t = rearrange(RGB_features8 , 'b c h w-> b (h w) c')
        SC_features5_t = rearrange(SC_features5 , 'b c h w-> b (h w) c')
        for i, blk in enumerate(self.blocks):
            x = blk(RGB_features8_t ,SC_features5_t , H, W)
        x = rearrange(x , 'b (h w) c-> b c h w', h=H,w=W)
        hx = self.attn_neck(x)
        hx = torch.cat([hx, measurement_feature], dim=1) 
        fuse = hx.clone()#NEW
