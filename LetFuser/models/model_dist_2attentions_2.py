...
class Fusion_Block(nn.Module):
    ...
    def forward(self, features, features_pooled, h, w, hp, wp):
        ...
        return x,attn_mixed,attn_pooled
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in, gt_command ):#, gt_ss, gt_redl:
       ...
       # fuse = hx.clone()#NEW
        ...
        for i, blk in enumerate(self.blocks):
            x,attn_mixed,attn_pooled = blk(features_cat,features_cat_pooled, H, W, Hp, Wp)

        x = rearrange(x , 'b (h w) c-> b c h w', h=H,w=W)
        hx = self.attn_neck(x)
        hxp = self.attn_neck_pooled(attn_pooled.permute(0,2,1).reshape(bs,-1,Hp,Wp)) 
