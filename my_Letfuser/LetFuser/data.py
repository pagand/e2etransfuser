...
#                            filename = f"{str(seq*self.seq_len+1+i).zfill(4)}.png"
                            ...
                            # position
                            with open(scenario_dir + f"/measurements/{str(seq+1+i).zfill(4)}.json", "r") as read_file:
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            ...
            data['depth_fronts'].append(torch.from_numpy(np.array(rgb_to_depth(
                scale_and_crop_image_cv(swap_RGB2BGR(cv2.imread(seq_depth_fronts[i],cv2.COLOR_BGR2RGB)), scale=self.config.scale, crop=self.config.input_resolution))))) #[ ]
